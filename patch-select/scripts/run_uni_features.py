...
def _load_tumor_summary(path: Path) -> dict[str, dict[str, Any]]:
    if not path.exists():
        return {}
    df = pd.read_csv(path, dtype={"slide_id": "string", "slide_uid": "string"})
...
def main():
    ...
    ap.add_argument("--slide-id", default=None, help="Optional single slide_id or slide_uid to run.")
    ap.add_argument("--overwrite", action="store_true", help="Overwrite existing done slides.")
    ap.add_argument("--multi-worker-mode", action="store_true", help="Enable worker overrides for patch IO prefetch.")
    ap.add_argument("--workers", type=int, default=None, help="CPU workers hint used when multi-worker mode is enabled.")
    ap.add_argument("--io-workers", type=int, default=None, help="Number of DataLoader workers for patch extraction from WSI.")
    ap.add_argument("--verbose", action="store_true", help="Enable detailed console logs.")
    args = ap.parse_args()
    ...
    logger, interactive = stage_logger("uni", uni_out_root, verbose=bool(args.verbose))
    ...
    logger.info("=== UNI SETUP ===")
    logger.info(f"Encoder: {encoder_name} checkpoint={encoder_ckpt}")
    logger.info(f"Model device: {model_device} | Norm device: {norm_device}")
    logger.info(f"Patch size: {out_patch_size} | Scale factor: {scale_factor} | Batch size: {batch_size}")
    logger.info(f"Multi-worker mode: {multi_worker_mode} | io_workers={io_workers}")
    ...
    if use_macenko:
        ...
        logger.info("Macenko normalizer: enabled")
    else:
        logger.info("Macenko normalizer: disabled")
    ...
    logger.info(f"Coord source: {coord_source} | use_uni_ready_only={use_uni_ready_only}")
    logger.info(f"max_patches_per_slide={max_patches} min_ok_patches_per_slide={min_ok_patches}")

    rows: list[dict[str, Any]] = []
    reporter = PeriodicProgress(logger, "uni", total=len(slide_items), every=25)
    for idx, slide_rec in enumerate(progress(slide_items, interactive=interactive, desc="[uni] slides", unit="slide"), start=1):
        slide_path = Path(str(slide_rec["path"]))
        center = str(slide_rec["center"])
        sid = str(slide_rec["slide_id"])
        slide_uid = str(slide_rec["slide_uid"])
        slide_relpath = str(slide_rec["slide_relpath"])
        t0 = time.time()
        row: dict[str, Any] = {
            "slide_id": sid,
            "slide_uid": slide_uid,
        ...
        tumor_row = tumor_lookup.get(slide_uid) or tumor_lookup.get(sid, {})
        ...
        row["mask_status_effective"] = str(qc_row.get("mask_status_effective", ""))

        out_slide_dir = uni_out_root / center / "uni" / slide_uid
        ...
        try:
            ...
            coord_center_root = tumor_out_root / center / "tumor_gate" / slide_uid
                    ...
                    for local_i, patch in enumerate(patches):
                        ...
                        try:
                            ...
                            if macenko is not None:
                                ...
                                arr = norm_t.permute(1, 2, 0).numpy() if norm_t.ndim == 3 and norm_t.shape[0] == 3 else norm_t.numpy()
                                if arr.max() <= 5.0:
            ...
            if ok_count == 0:
                ...
                row["elapsed_sec"] = float(time.time() - t0)
                rows.append(row)
                logger.warning(f"[uni] {slide_uid}: fail(no features)")
                continue

            feats_to_save = feats.astype(np.float16 if output_dtype == "float16" else np.float32, copy=False)
            ...
            reporter.update(idx, ok=sum(1 for r in rows if str(r.get("status", "")) == "ok"))

    run_df = pd.DataFrame(rows).sort_values(["center", "slide_id"]).reset_index(drop=True)
    ...
    if merge_run_summary and run_summary_path.exists() and len(run_df) > 0:
        ...
        subset = ["slide_uid"] if "slide_uid" in run_df.columns else (["center", "slide_id"] if "center" in run_df.columns else ["slide_id"])
    ...
    if len(run_df):
        ...
        logger.info(f"UNI feature ready: {ready}/{len(run_df)}")
