...
def _process_slide_record(
    ...
    *,
    ...
    if out_npy.exists() and not overwrite:
        return {"kind": "skip", "center": center, "row": None, "slide_uid": slide_uid}
    ...
    try:
        ...
        kind = "low_tissue" if status_eff == "low_tissue" else ("high_tissue" if status_eff == "high_tissue" else "ok")
        return {"kind": kind, "center": center, "row": row, "slide_uid": slide_uid}
        ...
        return {"kind": "error", "center": center, "row": error_row, "slide_uid": slide_uid}
    finally:
...
def main():
    ...
    parser.add_argument(
        "--multi-worker-mode",
        action="store_true",
        help="Enable slide-level parallel mask generation.",
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    def _consume_result(result: dict, done_count: int) -> None:
        kind = str(result.get("kind", "error"))
        center = str(result.get("center", ""))
        row = result.get("row")
        counts[kind if kind in counts else "error"] += 1
        if isinstance(row, dict):
            summary_rows.append(row)
            by_center_rows.setdefault(center, []).append(row)
        reporter.update(done_count, ok=counts["ok"], low=counts["low_tissue"], high=counts["high_tissue"], err=counts["error"], skip=counts["skip"])
    ...
    for rec in slide_records:
        slide_uid = str(rec["slide_uid"])
        center = str(rec["center"])
        out_npy = mask_dir / center / "mask" / f"{slide_uid}.npy"
        if out_npy.exists() and not overwrite:
            done_count += 1
            counts["skip"] += 1
            reporter.update(done_count, ok=counts["ok"], low=counts["low_tissue"], high=counts["high_tissue"], err=counts["error"], skip=counts["skip"])
    ...
    if multi_worker_mode and workers > 1 and len(pending_records) > 1:
        logger.info(f"Running mask generation in parallel across slides (workers={min(workers, len(pending_records))})")
        with ThreadPoolExecutor(max_workers=min(workers, len(pending_records))) as ex:
            futures = {
                ex.submit(
                    _process_slide_record,
                    rec,
                    mask_cfg=mask_cfg,
                    strategy=strategy,
                    mask_dir=mask_dir,
            ...
            for fut in progress(
                ...
                result = fut.result()
                done_count += 1
                _consume_result(result, done_count)
    else:
        for rec in progress(pending_records, interactive=interactive, desc="[mask] slides", unit="slide"):
            result = _process_slide_record(
                rec,
                mask_cfg=mask_cfg,
                strategy=strategy,
                mask_dir=mask_dir,
